...
public class Countries {
    ...
    public static Country getCountryByFirstSymbols(String symbols) {
        String formated = symbols
                .replace("+", "")
                .replace("-", "")
                .replace(" ", "");
        ...
        if (!formated.isEmpty()) {
            ...
            if (formated.length() >= 4) {
                formated = formated.substring(0, 3);
            }
            int dialCode = Integer.parseInt(formated);
            return getCountry(dialCode);
        } else {
            return null;
        }
    }

    private static Country getCountry(int dialCode) {
